...
<?php
    ...
    $galaxyD = array(
        ...
        'All the magic behind this game relies on moving images on the screen at very specific moments. Any point on the game screen has an X and Y coordinates that locate it. For instance, the player ship image is located at a specific part of the screen when the game starts, if we press the right arrow (->) of the keyboard we\'re going to add 10 to the ship image X coordinate moving it to the right. The thing is, we check if the location of any image on the screen needs to change because some event just happened then we redraw all images on the screen at their current location. That happens 60 times per second, that\'s why our game (a program that redraws images on the screen very quickly) seems to be very fluid. Examples of events can be the act of pressing a key (space bar, for example), or just time passing.',
